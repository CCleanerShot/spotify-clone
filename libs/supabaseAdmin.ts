...
const copyBillyingDetailsToCustomer = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
	...
	const { error } = await supabaseAdmin
		.from("users")
		.update({ billing_address: { ...address }, payment_method: { ...payment_method[payment_method.type] } })
		.eq("id", uuid);
...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction = false) => {
	...
	if (createAction && subscription.default_payment_method && uuid) {
		await copyBillyingDetailsToCustomer(uuid, subscription.default_payment_method as Stripe.PaymentMethod);
	}
};
